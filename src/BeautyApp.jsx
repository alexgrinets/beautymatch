...
const MASTERS = [
  {
    ...
    rating: 4.9, reviews: 312, distance: "0.8 км", price: "від 800 ₴",
    ...
    verified: true, address: "вул. Хрещатик, 14", promoted: true,
  },
  {
    id: 2, name: "Оксана Лисенко", specialty: "Нейл-майстер",
    rating: 4.8, reviews: 198, distance: "1.2 км", price: "від 550 ₴",
    ...
    verified: true, address: "вул. Саксаганського, 32", promoted: false,
  },
  {
    id: 3, name: "Марина Дорошенко", specialty: "Перукар-стиліст",
    rating: 4.95, reviews: 421, distance: "2.1 км", price: "від 1200 ₴",
    ...
    verified: true, address: "вул. Велика Васильківська, 77", promoted: false,
  },
  {
    id: 4, name: "Катерина Іванова", specialty: "Лешмейкер",
    rating: 4.7, reviews: 156, distance: "0.5 км", price: "від 700 ₴",
    ...
    verified: false, address: "пр. Перемоги, 5", promoted: false,
  },
  {
    id: 5, name: "Юлія Момот", specialty: "Косметолог",
    rating: 4.9, reviews: 287, distance: "3.0 км", price: "від 1500 ₴",
    ...
    verified: true, address: "вул. Золотоворітська, 2", promoted: true,
  },
];
...
export default function BeautyApp() {
  ...
  const [matches, setMatches] = useState([MASTERS[0]]);
  const [chats, setChats] = useState({
    1: { master: MASTERS[0], messages: [{ from: "master", text: "Привіт! Рада, що ми зматчились 💕 Чим можу допомогти?", time: "10:31" }] },
  });
  ...
  const master = MASTERS[cardIndex % MASTERS.length];
        ...
        <div style={S.overlay}>
          <div style={S.matchBox} className="pop">
            ...
            <div style={S.matchAvas}>
              ...
              <img src={matchOverlay.avatar} style={S.matchAva} alt="" />
            </div>
        ...
        <div style={S.overlay} onClick={() => setProfileModal(null)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={S.modalInner}>
              <img src={profileModal.portfolio[0]} style={S.modalCover} alt="" />
              <div style={S.modalBody}>
                <div style={S.modalHead}>
                  <img src={profileModal.avatar} style={S.modalAva} alt="" />
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <span style={S.modalName}>{profileModal.name}</span>
                      {profileModal.verified && <span style={S.badge}>✓</span>}
                    </div>
                    <p style={S.modalSpec}>{profileModal.specialty}</p>
                    <p style={S.modalMeta}>⭐ {profileModal.rating} · 📍 {profileModal.distance} · {profileModal.price}</p>
                  </div>
                </div>
                ...
                <div style={S.portGrid}>
                  {profileModal.portfolio.map((src, i) => <img key={i} src={src} style={S.portThumb} alt="" />)}
                </div>
        ...
        <div style={S.screen}>
          ...
          <div style={S.cardWrap}>
            <div key={cardIndex} className={`card-in ${swipeAnim === "right" ? "go-right" : swipeAnim === "left" ? "go-left" : ""}`} style={S.card}>
              {master.promoted && <div style={S.promo}>⚡ Топ майстер</div>}
              ...
              <div style={{ position: "relative", height: 460 }}>
                <img src={master.portfolio[photoIndex]} style={S.photo} alt="" />
                <div style={S.dots}>
                  {master.portfolio.map((_, i) => <div key={i} onClick={() => setPhotoIndex(i)} style={{ ...S.dot, background: i === photoIndex ? "#d4a07b" : "rgba(255,255,255,0.35)", cursor: "pointer" }} />)}
                </div>
                <div style={{ position: "absolute", top: 0, left: 0, width: "40%", height: "100%", zIndex: 4 }} onClick={() => setPhotoIndex(Math.max(0, photoIndex - 1))} />
                <div style={{ position: "absolute", top: 0, right: 0, width: "40%", height: "100%", zIndex: 4 }} onClick={() => setPhotoIndex((photoIndex + 1) % master.portfolio.length)} />
                <div style={S.grad} />
                <div style={S.info}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 2 }}>
                    <span style={S.cName}>{master.name}</span>
                    {master.verified && <span style={S.badge}>✓</span>}
                  </div>
                  <p style={S.cSpec}>{master.specialty}</p>
                  <div style={{ display: "flex", gap: 7, marginBottom: 8 }}>
                    {["⭐ " + master.rating, "📍 " + master.distance, "💳 " + master.price].map(v => <span key={v} style={S.chip}>{v}</span>)}
                  </div>
              ...
              {matches.slice(-5).map(m => (
                <div key={m.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: "pointer" }} onClick={() => { setActiveChat(m.id); setScreen("chat"); }}>
                  <div style={{ width: 66, height: 66, borderRadius: "50%", overflow: "hidden", border: "2.5px solid rgba(212,160,123,0.5)", flexShrink: 0 }}>
                    <img src={m.avatar} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" />
                  </div>
            ...
            {matches.map(m => (
              <div key={m.id} style={S.listItem} onClick={() => setProfileModal(m)}>
                <div style={{ position: "relative", flexShrink: 0 }}>
                  <img src={m.avatar} style={{ width: 52, height: 52, borderRadius: "50%", objectFit: "cover" }} alt="" />
                  <div style={S.online} />
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={S.ln}>{m.name}</span>
                    {m.verified && <span style={S.badge}>✓</span>}
                  </div>
          ...
          {!activeChat ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
              <div style={S.chatHead}>
                ...
                <img src={chats[activeChat]?.master.avatar} style={{ width: 40, height: 40, borderRadius: "50%", objectFit: "cover" }} alt="" />
                <div style={{ flex: 1 }}>
              ...
              <div style={S.msgs}>
                {chats[activeChat]?.messages.map((msg, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: msg.from === "user" ? "flex-end" : "flex-start", marginBottom: 10, alignItems: "flex-end", gap: 8 }}>
                    {msg.from === "master" && <img src={chats[activeChat]?.master.avatar} style={{ width: 28, height: 28, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} alt="" />}
